...
template <int D=DIM, typename T=double>
class vector {
  ...
  static double norm_squared(const vector<D, T>&, const vector<D, T>&);
...
template <int D, typename T>
inline const T& vector<D, T>::operator[](const unsigned int i) const
{
  return x[i];
}
...
// (a-b)^2
template <int D, typename T>
inline double vector<D, T>::norm_squared(const vector<D, T>& a, const vector<D, T>& b)
{
  double d = 0, dtmp;
...
// ~~~~~
template <int D, typename T>
vector<D, T>& vector<D, T>::setzero()
{
  for(int k=0; k<D; ++k) x[k]=0.0;
  return *this;
}
