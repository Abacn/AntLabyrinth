...
//  VoidPercolation
//
// Disjoint set of cell

#include "graph_construct_3d.hpp"

// Created as parent
Cell_disjoint_set::Cell_info::Cell_info():
parent(nullptr), rank(1), vec_to_parent(Vector(0.0,0.0,0.0))
{
}

// Created as child
Cell_disjoint_set::Cell_info::Cell_info(Cell_handle self, Cell_handle parent):
rank(0), parent(parent)
{
  vec_to_parent = dif_cells_PBC(self, parent);
}
...
uint32_t Cell_disjoint_set::merge(const Facet_decorate &fc)
{
  ...
  if(it1== cellset.end() || it1->first != cell1)
  {
    if(it2 == cellset.end() || it2->first != cell2)
    {
      ...
      it2 = cellset.insert(it2, CellMapType::value_type(cell2, Cell_info(cell2, cell1))); // add cell2 to cell1
      root0 = it1;
      newcluster(cell1);
    }
    else
    {
      ...
      it1 = cellset.insert(it1, CellMapType::value_type(cell1, Cell_info(cell1, cell2)));
  ...
  else
  {
    if(it2 == cellset.end() || it2->first != cell2)
    {
      ...
      it2 = cellset.insert(it2, CellMapType::value_type(cell2, Cell_info(cell2, cell1)));
    ...
    else
    {
      ...
      if(root1 != root2)
      {
        Vector dis1to2 = dif_cells_PBC(it1->first, it2->first);
      ...
      else
      {
        ...
        Vector disA = dif_cells_PBC(it1->first, it2->first);
...
Cell_disjoint_set::CellMapType::iterator Cell_disjoint_set::findset(CellMapType::iterator it)
{
  ...
  std::vector<CellMapType::iterator>::reverse_iterator itit;
  Cell_handle par = itp->second.parent;
  if(par == nullptr) return itp;
  do
  {
    route.push_back(itp);
  ...
  if(route.size() > 1)
  {
    ...
    for(itit = std::next(route.rbegin()); itit != route.rend(); ++itit)
    {
