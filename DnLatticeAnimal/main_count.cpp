...
int main(int argc, const char * argv[]) {
  ...
  if(argc < 3)
  {
    std::cout << "Usage: DnLatticeAnimal D N" << std::endl;
    return 1;
  }
  int D, N;
  D = atoi(argv[1]);
...
#define RUND(DIM, N) case(DIM):{PointNeighborDn<DIM> dn; CountLatticeAnimal<DIM> an(dn); auto result=an.getCount(N, preassignedidxs); printResult(result);} break;
  switch(D)
  {
      RUND(2, N)
      RUND(3, N)
